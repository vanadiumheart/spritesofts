...
minetest.register_entity("spritesofts:gunitem",{
	hp_max = 1,
	visual="wielditem",
	visual_size={x=.4,y=.4},
	...
	for list, data in pairs(lists) do
		...
		for i = 1, size do
			...
			if stackname then
				...
				if spritesofts.registered_guns[stacksub] and not spritesofts.registered_guns[stacksub].concealed then
...
local function get_largest(weapons)
	...
	for index, weapon in pairs(weapons) do
		if index ~= "wield" then
			local gunsize = spritesofts.registered_guns[weapon].space
...
local function update_weapon(player)
	...
	for index, gun in pairs(weapons) do
		gun = gun or ""
		local def = spritesofts.registered_guns[gun]
	...
	if not gunitems[name] then
		gunitems[name] = minetest.add_entity(player:get_pos(), "spritesofts:gunitem", minetest.serialize({owner = name, item = weapon}))
	else
...
minetest.register_on_player_inventory_action(function(player, action, inventory, inventory_info)
	...
	if not stack and action == "move" then
		...
		if not stack or not spritesofts.registered_guns[stack:get_name()] then
			stack = inventory:get_stack(inventory_info.from_list, inventory_info.from_index)
		end
	end
	if stack and spritesofts.registered_guns[stack:get_name()] then
