...
minetest.register_chatcommand("spritesoftscale",{
...
spritesofts.is_wielding_gun = function(name)
	return gun_huds[name] ~= nil
end

minetest.register_chatcommand("spritesoftskipstep",{
	params = "<scale>",  -- Short parameter description
...
local function set_physics(player, amount)
	if playercontrol then
		playercontrol.set_effect(player:get_player_name(), "speed", amount, "spritesofts", true)
	else
...
local function zoom(name, def, val)
	...
	if val then
		...
		if def and def.zoomfov then
			if playercontrol then
				local fov = playercontrol.set_effect(name, "fov", def.zoomfov, "spritesofts", false)
		...
		if playercontrol then
			player:set_fov(playercontrol.set_effect(name, "fov", nil, "spritesofts", false) or 0, false, .15)
		else
...
local function add_gun(name, gunname)
	...
	local def = spritesofts.registered_guns[gunname]
...
minetest.register_node("spritesofts:flash", {
	drawtype = "airlike",
...
local wagtbl = {}
function spritesofts.set_wag(name, value)
	if not name then return end
	wagtbl[name] = value
end
...
minetest.register_globalstep(function(dtime)
	...
	for _, player in pairs(minetest.get_connected_players()) do --todo only check on inv move or wield index change
		...
		if not gun_huds[name] then
			...
			local def = spritesofts.registered_guns[wield]
	...
	for name, tbl in pairs(gun_huds) do
		...
		local def = spritesofts.registered_guns[tbl.gun]
...
function spritesofts.register_gun(gunname, def)
	def.gunname = gunname
	if def.damage then
		local oldfunc
	...
	spritesofts.registered_guns[gunname] = def
end

function spritesofts.register_magazine(magazine, ammunition, size)
	minetest.override_item(magazine, {
    groups = {spritesofts_magazine=1},
...
minetest.get_craft_result = function(input)
	...
	for i, item in pairs(output) do
		item = ItemStack(item)
		local def = minetest.registered_items[item:get_name()]
		if def and def.groups and def.groups.spritesofts_magazine and def._ammunition_item then
			magazine = item
...
--]]
local mp = minetest.get_modpath("spritesofts")
dofile(mp.."/guns.lua")
